...
build_variant() {
  ...
  (cd "$root/green" && SIGNOZ_LIB_ROOT="$root" ./green build -f "$tmp/$variant-green.yml" >/dev/null)
  (cd "$root/red" && SIGNOZ_LIB_ROOT="$root/red" ./red build -f "$tmp/$variant-red.yml" >/dev/null)
  (cd "$root/blue" && uv run python -m package_signoz_blue build -f "$tmp/$variant-blue.yml" >/dev/null)
...
echo "green, red, and blue SigNoz artifacts are byte-identical"
