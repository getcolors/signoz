...
dir="<{ signoz-backup-dir }>"
bucket="<{ signoz-backup-r2-bucket }>"
profile="<{ profile }>"
...
export RCLONE_CONFIG_R2_ENDPOINT="<{ signoz-backup-r2-endpoint }>"
export RCLONE_CONFIG_R2_REGION="<{ signoz-backup-r2-region }>"

rclone copy "$file" "r2:$bucket/$profile/"

# This deployment owns only its own prefix. Retention never reaches outside it.
rclone delete --min-age <{ signoz-backup-retention-days }>d "r2:$bucket/$profile/"
find "$dir" -name 'metastore-*.sql.gz' -mtime +<{ signoz-backup-retention-days }> -delete
