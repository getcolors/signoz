...
import { signozWorkflow } from "./workflow.ts";
...
export const usage =
  ...
  "  create    provision and verify the SigNoz observability server\n" +
...
export async function run(...args: string[]): Promise<Opts> {
  ...
  if (lifecycleCommands.includes(command)) {
    return runCli(signozWorkflow, withFile);
  }
...
export async function exec(args: string[] = Bun.argv.slice(2)): Promise<never> {
  if (lifecycleCommands.includes(args[0] ?? "")) {
    return execCli(signozWorkflow, defaultArgs(args));
  }
