(ns io.github.getcolors.signoz.compute
...
(defn requirements [opts]
  (let [ssh (request/source-cidrs opts "ssh-sources" "signoz-ssh-sources") http (request/source-cidrs opts "http-sources" "signoz-http-sources")]
    {:single_host true :legacy_state_keys [(str (:profile opts) "/signoz-infrastructure.tfstate")]
...
(defn attach [opts result]
  (cond
    ...
    (= "destroyed" (:status result)) (assoc opts :green/exit 0 :signoz/already-destroyed true)
