...
(deftest library-node-params-feed-downstream-steps
 (let [p (tools/fallback-params (fixture))]
  (is (= "0" (:node_id p))) (is (nil? (:vpc_ip p))) (is (= "signoz-fixture" (:name p))))
 (is (str/includes? (tools/inventory (fixture :ip "203.0.113.7" :user "ubuntu")) "ubuntu")))
...
(deftest a-delete-without-compute-skips-the-host-entirely
  ...
  (is (= 1 (:green/exit (tools/ansible-step (assoc (fixture) :green/event :delete))))))
