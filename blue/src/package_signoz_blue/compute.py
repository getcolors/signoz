...
def requirements(opts):
    ssh = source_cidrs(opts, 'ssh-sources', 'signoz-ssh-sources')
    http = source_cidrs(opts, 'http-sources', 'signoz-http-sources')
    return {'single_host': True, 'legacy_state_keys': [opts['profile'] + '/signoz-infrastructure.tfstate'],
...
def attach(opts, result):
    ...
    if result['status'] == 'destroyed':
        return {**opts, 'blue/exit': 0, 'signoz/already-destroyed': True}
    params = node(result)
